...
function addPlant(latin, displayName) {
  ...
  const plants = store.plants;
  plants.push({ id: uid(), name, latin, photo: scanThumb, added: Date.now(), updatedAt: Date.now(), lastWatered: Date.now(), journal: [{ id: uid(), t: Date.now(), type: "added", ...currentJournalAuthor() }] });
  store.plants = plants;
  toast("🪴 Dodano: " + name);
